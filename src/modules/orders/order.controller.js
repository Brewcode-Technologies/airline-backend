...
const getOrders = async (req, res, next) => { try { success(res, await orderService.getAll(req.user)); } catch (e) { next(e); } };
const getOrder = async (req, res, next) => { try { success(res, await orderService.getById(req.params.id)); } catch (e) { next(e); } };
const createOrder = async (req, res, next) => { try { req.body.createdBy = req.user.id; res.status(201).json({ success: true, data: await orderService.create(req.body) }); } catch (e) { next(e); } };
